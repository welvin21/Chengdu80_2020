...
export const Portfolio = ({ container }) => {
  ...
  useEffect(() => {
    const fetchStocksByIndustry = async () => {
      const response = await fetch("http://localhost:5000/get-stock-data-by-industry");
      const stocksByIndustryData = await response.json();

      const processedOptions = Object.keys(stocksByIndustryData).map(industry => {
        const stocksForIndustry = stocksByIndustryData[industry];
        return {
          value: industry,
          label: industry,
          children: stocksForIndustry.map(stock => ({ value: stock, label: stock }))
        }
      });
  ...
  return (
    <div style={{ height: "100%", boxSizing: "border-box" }}>
      <Affix target={() => container}>
        <div style={{ padding: "12px", backgroundColor: "#001628", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Title level={4} style={{ margin: 0, color: "white",textAlign:"left"  }}>
            My Portfolio
          </Title>
          <Button type="primary" onClick={() => setWantsToAdd(true)}>Add</Button>
        </div>
        {
          wantsToAdd &&
          <div style={{ width: "100%" }}>
            <Cascader
              ...
              onChange={value => {
                ...
                if (!portfolio.some(stock => stock.ticker === stockToAdd)) {
                  localStorage.setItem("portfolio", JSON.stringify([{ "ticker": stockToAdd }, ...portfolio]));
