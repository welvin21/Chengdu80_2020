...
import { Typography, Card, Affix, Button, message, Empty, Spin, Tag, Tooltip } from "antd";
...
export const RecommendationsDiversify = ({ container }) => {
  ...
  useEffect(() => {
    const fetchDiversifyRecs = async () => {
      const processedPortfolio = portfolio.map(stock => `${stock.ticker}_return`);
      console.log(processedPortfolio);
      const response = await fetch("http://localhost:5000/get-diversify-recommendations", {
        method: "POST",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>
                    ...
                    <Tooltip title="correlation between your portfolio's average returns and this stock's return">
                      <Tag style={{ backgroundColor: "#001628", marginLeft: "8px", color: "white" }}>{score.toFixed(3)}</Tag>
                    </Tooltip>
                  </span>
                  <Button type="primary"
                    onClick={() => {
